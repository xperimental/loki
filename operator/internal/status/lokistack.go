...
func generateCondition(ctx context.Context, cs *lokiv1.LokiStackComponentStatus, k k8s.Client, req ctrl.Request, stack *lokiv1.LokiStack, degradedErr *DegradedError) (metav1.Condition, error) {
	...
	terminating := len(cs.Compactor[lokiv1.PodTerminating]) +
		len(cs.Distributor[lokiv1.PodTerminating]) +
		len(cs.Ingester[lokiv1.PodTerminating]) +
		len(cs.Querier[lokiv1.PodTerminating]) +
		len(cs.QueryFrontend[lokiv1.PodTerminating]) +
		len(cs.Gateway[lokiv1.PodTerminating]) +
		len(cs.IndexGateway[lokiv1.PodTerminating]) +
		len(cs.Ruler[lokiv1.PodTerminating])
