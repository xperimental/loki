	...
	},
	{
		...
		spec: lokiv1beta1.AlertingRuleSpec{
			Groups: []*lokiv1beta1.AlertingRuleGroup{
				{
					Name:     "first",
					Interval: lokiv1beta1.PrometheusDuration("1mo"),
				},
			},
		},
		err: apierrors.NewInvalid(
			...
			Groups: []*lokiv1beta1.AlertingRuleGroup{
				{
					...
					Rules: []*lokiv1beta1.AlertingRuleGroupSpec{
						{
							Alert: "an-alert",
							For:   lokiv1beta1.PrometheusDuration("10years"),
...
func TestAlertingRuleValidationWebhook_ValidateUpdate(t *testing.T) {
	for _, tc := range att {
		t.Run(tc.desc, func(t *testing.T) {
			...
			v := &validation.AlertingRuleValidator{}
			err := v.ValidateUpdate(ctx, &lokiv1beta1.AlertingRule{}, l)
			if err != nil {
				require.Equal(t, tc.err, err)
			} else {
				require.NoError(t, err)
